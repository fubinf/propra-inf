...
[EQ]  
Warum kann man mit dem Wert, der aus dem `localStorage` gelesen wird,  
...
1. Geben Sie `jsonText` mit `console.log` aus in der Konsole des Browsers.  
2. Vergleichen Sie die Ausgabe mit der ursprünglichen Datenstruktur.  
3. Erklären Sie kurz, warum `jsonText` kein Array mehr ist, obwohl er äußerlich noch Klammern enthält.
...
1. Geben Sie `arr` mit `console.log` aus.  
2. Greifen Sie auf `arr[0].text` zu und geben Sie den Wert aus.  
3. Fügen Sie dem Array mit `arr.push(...)` ein weiteres Objekt hinzu.
...
In Ihrer App passiert das immer dann, wenn Sie bisher nach einer Aktion
die Darstellung aktualisieren mussten.
...
Speichern allein reicht noch nicht: Wenn wir die Seite neu laden, startet JavaScript wieder von vorne.
...
Wählen Sie eine eindeutige `id` (zum Beispiel `resetBtn`) und einen passenden Text
(zum Beispiel „Alle Notizen löschen“).

Platzieren Sie den Button so, dass er zur Bedienung der App passt
(zum Beispiel in der Nähe des „Hinzufügen“-Buttons).
