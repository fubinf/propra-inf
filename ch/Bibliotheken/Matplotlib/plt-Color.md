...
[ER] Setzen Sie mit 
[`set_prop_cycle`](https://matplotlib.org/stable/api/_as_gen/matplotlib.axes.Axes.set_prop_cycle.html)
eine eigene *ColorSequence* fest, sodass alle Plots standardmäßig in den Farben lila, grün, grau
erscheinen (in dieser Reihenfolge).  
Plotten Sie erneut drei Funktionen (ohne `color` anzugeben) und beobachten Sie die Reihenfolge.

[ER] Erstellen Sie einen Scatter-Plot mit 100 Punkten, deren x- und y-Wert zufällig ist 
(zwischen 0 und 100).  
...
- links: eine sequentielle Colormap (`'plasma'`)  
- rechts: eine diskrete Colormap, suchen Sie sich dafür eine qualitative Colormap heraus.

[NOTICE]
Siehe auch die Übersicht über alle Colormaps: [`plt.colormaps()`](https://matplotlib.org/stable/tutorials/colors/colormaps.html)
[ENDNOTICE]

`alpha` ist ein Parameter, der in vielen Plotting-Bibliotheken wie Matplotlib verwendet wird, um
die Transparenz von grafischen Elementen wie Punkten, Linien oder Flächen zu steuern. 
Der Wert von alpha liegt typischerweise zwischen 0 und 1, wobei `alpha=0` das Element vollständig
transparent (unsichtbar) macht und `alpha=1` es vollständig undurchsichtig darstellt.
Besonders bei vielen graphischen Elemente, die sich überlappen, kann es sinnvoll sein, den `alpha`-
Wert zu bearbeiten.

[ER] Erstellen Sie einen Scatter-Plot, bei dem die Transparenz (`alpha`) 
mit den y-Werten variiert.  
Mit `vmin` und `vmax` lässt sich der Wertebereich für die Colormap festlegen.
Setzen Sie `vmin=0`, `vmax=5` für die Colormap `viridis`, um die Farbskala zu fixieren.  

Manchmal ist es praktisch, die Farben einer Colormap invertieren ("umdrehen") zu können.  
