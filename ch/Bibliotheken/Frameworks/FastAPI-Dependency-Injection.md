...
[SECTION::goal::idea]
Ich kann eine Abhängigkeit (Dependency) erstellen, um gemeinsame Funktionalität
...
Abhängigkeiten zentral zu lösen, um dies nicht in mehreren Endpunkten wiederholen zu müssen.

In FastAPI gibt es mit dem Konzept der _Dependency Injection_ einen komfortablen Weg,
etwas mehreren Endpunkten zugleich bereitzustellen.
_Dependency Injection_ bedeutet, dass ein Endpunkt bestimmte Werte oder Objekte
...
Kopieren Sie als Startpunkt Ihren Quellcode aus der Aufgabe [PARTREF::FastAPI-GET]
...
wenn die Antwortliste viele Einträge enthält.
Wenn eine Antwort maximal 100 Einträge pro Seite enthält, wird die Antwortzeit
deutlich reduziert.
...
Passen Sie die Logik ihres Endpunkts so an, dass nur der entsprechende Ausschnitt
...
[HINT::Slicing in Python]
`mylist[2:10]` gibt eine Teilliste ("slice") der Einträge an den Positionen 2 bis 9(!)
zurück.  
[HREF::https://docs.python.org/3/reference/expressions.html#slicings]
[ENDHINT]
...
Probieren Sie den Endpunkt aus und testen Sie unterschiedliche Werte, 
wie beispielsweise sehr große oder negative Zahlen.

Sie sehen, dass es z.B. möglich ist, `limit` auf `1000000` zu setzen.
...
Lesen Sie in der FastAPI Dokumentation, wie Sie
...
Da `fake_db` sehr klein ist, setzen Sie `page` auf `0` und `limit` auf `2`.
Wobei `page` größer gleich `0` sein sollte und `limit` größer `0` und kleiner gleich `10`.

Probieren Sie Ihren Endpunkt und die Parameter mit Grenzwerten aus.


### Dependency Injection

Nun haben Sie bei einem Endpunkt erfolgreich _Pagination_ implementiert.
Um das gleiche Verhalten auf weitere Endpunkte zu übertragen, müssten Sie jetzt die
Parameter kopieren und bei einem anderen Endpunkt einfügen.
Wenn Sie daraufhin Grenzen oder die Standardwerte ändern wollten, müssten Sie dies bei allen
...
[HINT::Wie berechnet man `start` und `end`?]
Echt jetzt? Das bekommen Sie doch selber raus!

[HINT::Nee, leider nicht.]
```python
start = page * limit
end = start + limit
```
...
[EQ] Nennen Sie eine weitere Nutzungsmöglichkeit der Dependency Injection im Programm.

[HINT::Ähh...]
In der Doku, die Sie schon gelesen haben, gibt es eine Liste.
[ENDHINT]
...
Beenden Sie Ihre Server. Dazu können Sie in der Konsole, in welcher der Server
...
[SECTION::submission::reflection,program,trace]
