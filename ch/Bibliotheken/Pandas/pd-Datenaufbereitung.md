...
umgehen kann, betrachten wir hier.
[ENDSECTION]
...
bereitstellt.
Wer dort rumstöbert, wird wahrscheinlich schnell überfordert sein mit der Menge an
...
[EQ] Die anderen Spalten (`QN_4 ... MX_RS`) haben kryptische Namen und daher ist nicht direkt
...
Was steht in den Spalten `MO_N`, `MO_TT`, `MO_FK` und `MO_RR`?

[NOTICE]
Die Spalte `eor` wird nicht erklärt.
Sie ist ein Indikator um das Zeilende ("end of row").
[ENDNOTICE]

Wie Sie merken, sind Metadaten unverzichtbar, um Daten zu verstehen, die man noch nicht kennt.
Sie wissen nun also, dass es sich hierbei um regelmäßige Messungen der genannten Werte handelt.

[EQ] Kaum zu glauben, dass alle diese Werte seit 1719 gemessen worden sein sollen. 
...
[EQ] Können Sie Ihre Vermutung mithilfe der Metadaten bestätigen? Mit welcher Datei?


# Spalten bearbeiten

[ER] Dass die Metadaten Auskunft über die Spaltennamen geben, ist erstmal schön und gut.
Beim Arbeiten mit diesen Daten ist es allerdings lästig, ständig nachzuschlagen, wofür "QN_4"
überhaupt steht. 
Erstellen Sie ein Dictionary, das den Spaltennamen  (`QN_4...MX_RS`) aussagekräftige Namen zuordnet. 
Eine sinnvolle Benennung für das Mittel der Minimum-Lufttemperatur in Celsius wäre z.B. "Lufttemp_C_min_mean".
...
[ER] Entfernen Sie `STATIONS_ID` aus dem `wetter_df` mit der Methode 
[`drop()`](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.drop.html#pandas.DataFrame.drop).

[EQ] Sind die Spalten `QN4` und `QN6` relevant für uns oder sollten wir sie ebenfalls löschen?
Begründen Sie.

Nun haben Sie den Datensatz kennengelernt und mithilfe der Metadaten verstanden.
Sie haben außerdem angefangen, den `DataFrame` zu formattieren, indem Sie Spalten lesbar umbenannt
haben und irrelevante Spalten gedropped haben.
