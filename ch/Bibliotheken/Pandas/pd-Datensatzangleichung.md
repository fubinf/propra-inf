...
Solche Daten liegen oft nicht in gleicher Struktur vor, weshalb man sie zuerst anpassen muss.
[ENDSECTION]
...
Dies ist der Download für die ausgewählten Daten, die die Website bereitstellt 
...
Zeitzone arbeitet und die andere "timezone-aware" also mit Zeitzone arbeitet.
...
[ER] Benennen Sie die restlichen Spalten so um, dass sie den gleichen Namen haben, wie die 
...
[ER] Passen Sie das `wetter_dahlem_df` so an, 
dass es nur Daten aus dem gleichen Zeitraum enthält, wie `wetter_aigen_df`.
Vergessen Sie nicht, die Indizes auch anzugleichen (`reset_index()`)!


### Vergleiche

[EQ] Bei welcher der zwei Stationen war es durchschnittlich kälter?
