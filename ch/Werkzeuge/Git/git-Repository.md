...
In [PARTREF::Git101] haben Sie gerade so das Nötigste gelernt, um Ihre Aufgaben im ProPra 
abzugeben. 
Jetzt geht es darum, das Gelernte zu vertiefen und Ihr Wissen über die Befehle und 
die [PARTREF::git-Funktionsweise] zu festigen.

In dieser Aufgabe erstellen Sie ein frisches Repository und lernen dabei,
...
zusammenhängen.
Am Ende steht Ihr erster Commit.
[ENDSECTION]
...
Ihr neues Repo wirkt auf den ersten Blick leer – sowohl im GUI-Dateimanager des 
...
* Das Arbeitsverzeichnis (Working Directory) ist Ihre lokale Arbeitskopie des Repos. 
  Hier können Sie beliebig Änderungen vornehmen.
* Diese Änderungen sowie neu hinzugefügte Dateien und Verzeichnisse können Sie dem Repository 
...
Besonders wichtig ist der `.git`-Ordner (Repository-Verzeichnis).
Zwar müssen Sie im Idealfall nie direkt in diesem Ordner arbeiten, 
...
[EQ] Wie heißt der Default-Branch bei Ihnen?
...
Erinnern Sie sich an das Archiv.
Git archiviert nur die Zustände des Repos, die Sie ihm *übergeben*.
...
Beginnen Sie mit einem kleinen Beispielprojekt – einem simplen Taschenrechner.
...
kümmert sich aber erst dann aktiv um sie, wenn Sie es dazu auffordern.
...
Jetzt können Sie Ihren ersten Commit erstellen:
