...
und gibt Ihnen volle Kontrolle darüber, was auf dem System läuft.
[ENDSECTION]
...
- `pgrep` unter macOS unterstützt die Option `-c` nicht;
...
Erstellen Sie zunächst ein einfaches Test-Skript, das für eine Weile läuft und Ausgaben produziert.

Erstellen Sie die Datei `Prozessmanagement.sh` in Ihrem [TERMREF::Hilfsbereich]
...
Wenn Sie ein Programm im Hintergrund ausführen möchten, können Sie an den Befehl ein `&` anhängen.
Allerdings schreibt ein Prozess im Hintergrund standardmäßig weiterhin in Ihr Terminal,
was fortlaufende Eingaben unübersichtlich macht und den Prompt stört.
...
`nohup` und `&` kombinieren Sie einfach damit.
...
Um die Kontrolle zu behalten, ist es wichtig zu wissen, welche Prozesse gerade laufen,
wer sie gestartet hat, wie viel Speicher und CPU sie verbrauchen und vor allem: 
wie sie (wenn nötig) beendet werden können.
...
Lesen Sie zum Verständnis den Abschnitt **bis einschließlich "Prozesse anzeigen"**
des Beitrages
[Prozessverwaltung](https://docs.rockylinux.org/latest/de/books/admin_guide/08-process/#allgemeines)
sowie die Abschnitte **DESCRIPTION** (zu UNIX- vs. BSD-Optionsstilen)
und **EXAMPLES** der Manpage
[ps(1) man](https://man7.org/linux/man-pages/man1/ps.1.html).
...
[EC] Zeigen Sie alle Prozesse im System im System-V-Format an (mittels `ps -ef`),
...
Das Verständnis von Prozessattributen ist praktisch, um schnell zu sehen, was ein Prozess tut.

Lesen Sie zum Verständnis die Abschnitte **PROCESS STATE CODES**
und **STANDARD FORMAT SPECIFIERS** der Manpage
[ps(1) man](https://man7.org/linux/man-pages/man1/ps.1.html).
...
Beobachten Sie die obere Leiste und die Prozessliste.
...
sowie **Synopsis** und **Syntax** aus der
...
[EQ] Warum sollte man das Signal `TERM` grundsätzlich bevorzugen und `KILL` nur im Notfall anwenden?
...
Lesen Sie den **FIELDS**-Abschnitt (insbesondere `%CPU`, `%MEM`, `TIME`, `COMMAND`, `RES`, `VIRT`)
aus der
[top(1) manpage](https://man7.org/linux/man-pages/man1/top.1.html).
