...
In diesem Programmierprojekt ist die Pogrammiersprache freiwählbar.
Im Rahmen dieses Projekts müssen JSON-Daten verarbeitet, im Dateisystem gespeichert
und eine REST-API bereitgestellet werden.
Hierbei sollte eine Sprache genutzt werden, in der Sie diese Anforderungen umsetzten können.
...
Erstellen Sie dort einen neuen Ordner `blockchain_example`.
Initialisieren Sie in dem Ordner ein neues Git-Repo und erstellen Sie eine Datei
...
Das Verzeichnis erscheint nach dem Rebase genauso aus wie vorher,
...
Inwiefern und auf welche Art das in Bitcoin und Sedricoin geschtütz wird,
...
Sedricoin soll vereinfacht als Client-Server-Architektur entwickelt werdne.
...
[NOTICE]
Durch die freie Entscheidung der Programmiersprache, gibt es keine Vorgaben zur
...
Die angegebene Variablen überschreiben immer die Standardwerte, falls Sie diese anlegen.
Ihrem Implementierung soll diese Umgebungsvariablen aktzeptieren:

```env
COIN_STORAGE_PATH="/pfad/zu/den/sedricoin/blöcken/"
```

Außerdem muss zu speicherne JSON-Datei (Modell) definiert werden.
...
3. Leerzeichen und andere `Whitespaces` entfernen.
...
5. Der Block-Hash ist die Hex-Repräsentation des Ergebnis (`UTF-8` Enkodierung).
