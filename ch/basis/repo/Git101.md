...
Nun machen legen wir unsere Änderung im Git Repo ab, wir erzeugen also eine neue
Version des Repos. 
Dieser Vorgang nennt sich Festschreibung (englisch: "commit")
und alle Welt verwendet fast immer nur den englischen Ausdruck.
Der Befehl dazu heißt natürlich `git commit`. 

Damit Git weiß, welche Dateien "committet" werden sollen, müssen wir diese zuvor 
auf eine Art Schmierzettel (genannt der "Index") hinzufügen.
...
Hier im Programmierpraktikum gehört dazu z.B. stets der Name der Aufgabe,
...
Dabei steht `-m` für `message` (Ohne die Option würde ein Editor gestartet,
über den man dann auch längere Commit-Nachrichten angeben könnte.
Das behandeln wir später.)
Führen Sie dieses Kommando jetzt durch.
...
Dazu dient `git log`. 
Rufen Sie das jetzt ohne weitere Parameter auf und studieren Sie sorgfältig die Ausgabe.
...
[HINT::Hinweis zur Praktikabilität]
  ...
  wenn man keinen `ssh-agent` laufen hat. Den richten wir später ein, in [TAL::GitSshAgent].
[ENDHINT]
...
- Ihre ProPra-Partner_in hat etwas hochgeladen, dass Sie nun zu sich abrufen, um es Ihrerseits
  zu einem eigenen Commit zuzufügen, z.B. ein Programm, das Sie beide identisch abgeben.
...
Kontrolliert wird nur, ob Ihre bisherigen Commits aussehen, wie sie sollen.

[INSTRUCTOR::Name, Email, Commit-Nachricht]
Nur prüfen, ob der Commit ungefähr wie gewünscht aussieht: README.md, Commit-Nachricht.
[ENDINSTRUCTOR]
