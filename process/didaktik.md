...
Im Folgenden soll ein Konzept vorgestellt werden zum Erstellen von Aufgaben im Rahmen des Programmierpraktikums.
Dabei wird ein Ablauf für die Aufgabenerstellung vorgeschlagen, 
...
Da das ProPra hauptsächlich das Ziel des Kompetenzzuwachses hat, 
...
**Leitfragen:**
 * Welche Aufgaben sollen die Studierenden bearbeiten, sodass das Lernziel erreicht wird?  
...
Als Ergebnis dieses Schritts soll eine Auflistung der Items aus Schritt 1 entstehen, 
...
> Zusätzlich soll auch herausgearbeitet werden, 
> was für welchen Verwendungszweck geeignet sein kann, insofern kommt dann noch "vergleichen" hinzu.
> Ergebnis in Ich-Form:
...
Eine Recherche zum Thema muss danach durchgeführt werden. 
Für das ProPra ist es fast immer notwendig, auf andere Quellen zu verweisen, 
die die Inhalte, die vermittelt werden sollen, strukturiert wiedergeben können. 
Gleichzeitig ist es hilfreich zu sehen, wie andere das Thema didaktisch aufbereitet haben, 
sodass Sie sich daran orientieren können. 
Sie müssen das Rad nicht für jede Aufgabe neu erfinden.

Außerdem gibt es zu bestimmten Themen auch oft vorgefasste Meinungen. 
...
1. Wie wird das Thema von Dritten vermittelt?
2. Gibt es geeignete Quellen, mit denen die Studierende Inhalte erarbeiten können?
3. Gibt es Aspekte oder Inhalte, um die Sie Ihr Wissen ergänzen müssen?
4. Welche Vorstellungen könnten die Studierenden zum Thema haben?
5. Welche Aspekte sind wichtig, um das intendierte Lernergebnis zu erreichen? Welche können Sie weglassen?
...
> Das entspricht Schritt 1 des Modells.
> 
...
> 
> In Aufgabe 2 und 3 wird dann Schritt für Schritt das Lernprodukt erstellt, 
> indem die Studierenden Tabellen in HTML bauen müssen 
> und ebenfalls mithilfe einer Quelle auf die Möglichkeit von Spalten oder Zeilen überspannenden Zellen eingegangen wird. 
> Aufgabe 3 macht dabei Gebrauch von Hints, um den Studierenden eine Lösungsfindung zu erleichtern.
> 
> Auch wenn Aufgabe 3 schon einen teilweise wiederholenden Charakter hat, 
> um Schritt 5 und 6 des Modells noch besser abzubilden, 
> wäre es bei dieser Aufgabe sicherlich sinnvoll noch eine Aufgabe zu ergänzen. 
...
Anschließend wird das Problem analysiert und die Lernenden gestalten die Lösung. 
Zum Schluss gilt es das Gelernte zu mit Vorwissen zu verknüpfen oder zu reflektieren. 
Die vier Phasen der Methode sollten dabei zeitlich in etwa alle denselben Raum einnehmen.

1. **Benutzen:**
   Vorstellung des Problems, indem ein Produkt oder Informatiksystem angewandt wird.
   Die Studierenden sollten hierbei auf ein Problem oder Sachverhalt aufmerksam werden, 
   der in Folge bearbeitet werden muss. 
   Während der Kontext im Hintergrund-Bereich der Aufgabe angerissen werden kann, 
   sollte das Benutzen eine Aufgabe sein, die die Studierenden durchführen müssen.
2. **Analysieren:**
   In diesem Schritt gilt es herauszufinden, was das Problem verursacht und welche Wege es möglicherweise gibt, 
   eine Lösung zu finden. 
   Vergleiche zu anderen Systemen (z.B. Python, wenn die Aufgabe zu einer anderen Programmiersprache ist), 
   Verweise auf Literatur zum Thema, Lesen von Fehlermeldungen 
   oder Verwenden von Vorwissen aus vorherigen Aufgaben sind sinnvolle Wege die Analyse zu gestalten und zu unterstützen.
3. **Gestalten:**
   Als dritten Schritt sollen die Studierenden, nachdem sie das Problem analysiert haben, mit den Methoden, 
   die sie in der Analyse-Phase gewonnen haben, das Problem auch lösen. 
   Für gewöhnlich ist dies eine relativ offene Aufgabenstellung, die den Studierenden selbst überlässt, 
...
> wo sie zunächst zuvor gelerntes Anwenden und die CSS-Definition um eine neue Eigenschaft (float) ergänzen. 
> Dabei entstehen typische Layoutprobleme: Die Bilder stapeln sich an einer Seite, das Layout bricht. 
> Die Website ist bis auf einige CSS-Definitionen komplett vorgegeben, 
> sodass die Lernenden sich auf das CSS konzentrieren können. 
> Für Flexbox und Grid sind in der Aufgabenstellung Layouts im Aussehen vorgegeben, das Problem ist dann zu erkennen, 
> wie dieses Aussehen durch Verwendung der Layouttechniken erreicht werden kann.
> 2. In der zweiten Phase (Analysieren) sollen die Lernenden nun mithilfe einer Quelle bei SelfHTML recherchieren 
> und herausfinden, wie das Layout durch Anwendung von `clear` und Clearfix nicht mehr bricht. 
> Für Flexbox und Grid wird ebenfalls je eine hochwertige Quelle zur Verfügung gestellt.
> 3. Anschließend sollen die Studierenden in der dritten Phase (Gestalten) eine mögliche Lösung einbauen. 
> Die Gestaltungsphase wird dabei mit Hints unterstützt. 
> Die Aufgaben sind bewusst offen formuliert, indem kein konkreter Lösungsweg vorgegeben ist, 
> sodass die Lernenden selbst entscheiden dürfen und müssen, wie sie zum vorgegebenen Ziel kommen.
...
Halboffene Aufgaben sind solche, bei denen ein Zielprodukt definiert ist, 
das aber weiterhin auf verschiedenen Lösungswegen erreicht werden kann. 
Diese können auch verschieden komplex sein. 
Das macht das Bewerten der Aufgaben natürlich auch komplexer. 
Nichtsdestotrotz gibt es Vorteile sich mit offenen Aufgaben zu beschäftigen:

- Selbstdifferenzierung: Die Lernenden wählen einen Lösungsweg, der ihrem Leistungsniveau entspricht.
- Selbstvertrauen: Die Lernenden erstellen ein Lernprodukt ohne eine Schritt-für-Schritt Anleitung.
- Kreativität: Die Lernenden können eigene Ideen umsetzen.

Aufgaben werden offener, indem sie produktorientiert gestellt werden 
und Hinweise zu Lösungsansätzen zunächst entfernt werden. 
