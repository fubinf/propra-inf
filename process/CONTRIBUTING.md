# For course authors: How to write tasks for a ProPra
...
## 2. Prior knowledge we assume students to have
...
## 3. Learning to write ProPra task content
...
### 3.2 Use the task file template
...
### 3.3 Design for difficulty level
...
### 3.5 Strive to re-use known didactic methods
...
### 3.6 Work steps towards producing tasks and task groups 
...
1. Outline a task description by selecting the section structure, 
   in particular the section subtypes (see 3.8 to 3.11),
   and formulating the learning goals (see 3.7 and [Didaktik 2.1](didaktik.md)).
...
3. Draft the background (keep it short) and instructions.
   ...
   See this document (3.8 to 3.11) for the practical execution.
...
5. Have someone else review the description for coherence between goal and instructions/submission
   and for appropriateness of the instructions given the difficulty. (see 6.5)
...
### 3.8 `[SECTION::background::default]`
...
### 3.10 `[SECTION::submission::subtype1,subtype2]`
...
### 3.12 Language use (in German because we use German)
...
### 6.1 First-time git setup (once per clone)

ProPra's use of `sedrila` implies we work on Linux or macOS.
Windows users need to set up a WSL.

```sh
git clone git@github.com:fubinf/propra-inf.git
cd propra-inf
git submodule update --init
./cmd/git-setup.sh          # applies team-wide git settings
```

`git-setup.sh` configures git the way the whole team needs it.
These settings cannot ship in a tracked config file — git ignores repo-supplied
config for security — so each clone must run the script once
...
It stores all text with **LF** in the repo and working tree, on every platform.
Do not override it with a personal `core.autocrlf`.
...
`altdir` (instructor solutions) is a separate repo that the main repo (the tasks)
pins to one exact commit — a "gitlink", not a branch.
`git-setup.sh`, called just once, configures push and pull so the two stay consistent:
the main repo *rebases* its own commits (nothing external references them),
while `altdir` *merges* — rebasing would give its commits new SHAs and break the
gitlinks the main repo has already recorded.

So you only need one habit:

> **One logical step = one `altdir` commit + one main-repo commit.
> `pull` before you start, `push` right after.**

Short edit episodes are the whole secret to avoiding submodule trouble.
A good IDE (such as PyCharm) does most of the bookkeeping for you.

**If it breaks anyway:**

- After `git pull`, the submodule sits at a **detached HEAD** (the main repo
  references a specific commit, and that commit is now checked out).
  To resume working on it: `(cd altdir; git switch main; git pull)`.
- If `git pull` in the main repo fails with
  "cannot rebase with locally recorded submodule modifications", run  
  `git pull --rebase --no-recurse-submodules`  
  `git submodule update --recursive`  
  (background: see the answer by Prechelt at
  https://stackoverflow.com/questions/54215983).
- If commits got stranded on a detached HEAD, note their commit IDs, do the
  cleanup above, then `git cherry-pick` them back.
  Treat `git reset --hard` with caution. When in doubt, ask for help.
...
- We integrate local commits via rebase, not via merge.
  The above call to `git-setup.sh` makes this the default behavior.
...
#### 6.5.1 Three stages of reviewing
...
#### 6.5.2 Modifying published tasks
...
- Larger, backwards-incompatible changes such as more steps, different timevalue,
  or even splitting a task into two must wait until the next course and are stored for
  later application in the manner described in
  "Using the `future-changes` branch" below.


### 6.6 Task text layout conventions
...
### 6.7 Using the `future-changes` branch
